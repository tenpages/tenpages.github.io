...
class Estimator():
    def __init__(self, env, learning_rate = 0.02):
        self.state = tf.placeholder(tf.float32, [None, env.observation_space.shape[0]], name="state")
        ...
        self.selected_action_prob = tf.diag_part(tf.gather(self.action_probs, self.action, axis = 1))

        self.loss = -tf.reduce_sum(tf.multiply(tf.log(self.selected_action_prob), self.G))
...
def generate_samples(env, estimator, learning_rate = 0.001):
    """
...
def estimate_return(episode, estimator, discount_factor = 0.1):
    """
...
def update_policy(episode, returns, estimator):
    ...
    total_loss = estimator.update_policy(states, returns, actions)
